...
export default function ResourcesPage() {
  return (
    <PageShell title="Resources" lead={resourcesMeta.intro} note={resourcesMeta.note}>
      <div className="mt-12 flex flex-col gap-12">
                ...
                <ul className="flex flex-col">
                  {group.items.map((r) => (
                    <li key={r.href}>
                      ...
                      >
                        {r.label}
                        <ArrowIcon className="h-4 w-4 shrink-0 -rotate-45 text-ink-3" />
                      </a>
                    </li>
                  ))}
                </ul>
        ...
        <section className="flex flex-col gap-5">
          ...
          <p className="max-w-prose text-meta text-ink-3">
          ...
          <ul className="flex flex-col">
            {agingSections.map((s) => (
              <li key={s.title} className="flex flex-col gap-1 border-t border-border py-4">
                ...
                <span className="text-meta text-ink-2">{s.summary}</span>
              </li>
            ))}
          </ul>
        </section>
