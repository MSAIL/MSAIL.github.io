...
export default function ResourcesPage() {
  return (
    <PlaceholderPage title="Resource Library" lead={resourcesMeta.intro} note={resourcesMeta.flag.note}>
          ...
          <div className="grid gap-8 sm:grid-cols-2">
            {learningConcepts.map((group) => (
              <div key={group.title} className="flex flex-col gap-3">
                <h3 className="text-label font-semibold uppercase tracking-[0.12em] text-maize-deep">
                  {group.title}
                </h3>
                  ...
                  {group.items.map((r) => (
                    <li key={r.href}>
                      <a
                        ...
                        className="group flex items-center justify-between gap-3 border-t border-border py-2.5 text-body text-ink transition-colors hover:text-maize-deep"
                      >
                        {r.label}
